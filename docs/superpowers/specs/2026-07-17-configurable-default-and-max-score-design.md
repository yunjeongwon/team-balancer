...
현재 `7`은 그래프 로직·LLM 프롬프트 어디에도 없고 점수 관리 페이지 두 곳
([2_점수_관리.py:18](../../../app/pages/2_점수_관리.py#L18) 안내 문구,
[2_점수_관리.py:58](../../../app/pages/2_점수_관리.py#L58) `max_value`)에만 있다.
...
기존 조회 표 아래, "추가 / 수정" 위에 **"기본 설정"** 섹션을 추가한다.

```
─── 기본 설정 ──────────────────────
미등록 인원에게 적용할 점수  [ 4 ]   (min_value=1, max_value=max_score)
최고 점수                    [ 7 ]   (min_value=1, max_value=20 고정)
[저장]
```

- 두 값 모두 `load_settings()`로 초기값을 채운다.
- "추가 / 수정" 폼의 `number_input`은 `max_value`를 하드코딩 `7` 대신
  `load_settings()["max_score"]`로 바인딩한다. 안내 문구의 "1~7"도 동적으로 만든다.
...
- `max_score`를 현재보다 낮춰서 **기존에 그 값을 초과하는 점수를 가진 선수가 있으면**,
  ...
  **기존 점수는 자동으로 깎지 않는다** — 자동 하향은 되돌릴 수 없는 데이터 손실이다.
  저장된 초과 점수는 그대로 유지되고, 이후 입력 폼에서만 새 상한이 적용된다.
