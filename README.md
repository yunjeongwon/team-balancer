...
> **모두 루트 레벨 키로(평평하게) 작성하세요.** Streamlit은 루트 레벨 secret을 자동으로 `os.environ`에도 주입합니다. `[section]` 안에 넣으면 env로 노출되지 않아, langchain이 `GOOGLE_API_KEY`/`OPENAI_API_KEY`를 찾지 못해 앱이 죽습니다.

> `GITHUB_TOKEN`은 점수 저장에 필요합니다 — 배포 앱이 `scores-data` 브랜치의 `data/scores.json`을 GitHub Contents API(PUT)로 갱신합니다. 권한은 classic PAT의 `repo` 스코프, 또는 fine-grained 토큰의 `Contents: Read and write`(이 리포 한정). 로컬 `.env`에 있는 토큰을 그대로 재사용해도 됩니다. 빠지면 점수 저장이 ephemeral 파일시스템에만 쓰고 재시작 시 날아갑니다.
...
점수는 배포된 앱의 **점수 관리 페이지**에서 직접 추가/수정/삭제합니다. 저장하면 GitHub API로 `scores-data` 브랜치의 `data/scores.json`에 자동 커밋되어, 앱이 재시작·재배포돼도 점수가 유지됩니다. `master` 브랜치는 건드리지 않아 점수 변경으로 인한 재배포는 일어나지 않습니다.
